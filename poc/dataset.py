...
class MVTecDataset(Dataset):
    """
    Loads MVTec AD images for one category.
    split: 'train' (only good) or 'test' (good + defects)
    Returns (image_tensor, mask_tensor, label, defect_type)
    ...
    def _load_samples(self):
        ...
        for defect_type in sorted(os.listdir(split_dir)):
            ...
            for img_path in sorted(defect_dir.glob("*.png")):
                if label == 1:
                    # ground truth mask has same name under ground_truth/
                    mask_path = (
